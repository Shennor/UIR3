...
class Requirements:
    ...
    __recognized_lit_templates = ['ГОСТ Р 7.0.100 -2018','ГОСТ Р 7.0.5 -2008','ГОСТ Р 7.0.5 --2008',
        'ГОСТ Р 7.0.5 2008','Ванкуверский стиль','Vancouver','Harvard','Council of Biology Editors',
        'GSA','MLA','MHRA', 'Chicago','AMS','ASME','AMA','NLM','APA','APSA','ASABE','ASA','AMA','AIP','IEEE','ALWD','NISO Z39.29-2005']
    ...
    __recognized_max_inverted = ['не' + sub1 + sub2 for sub1 in [' ', ' должен быть ', ' должен '] for sub2 in ['бол','выш','длинн','превыш','превосход']] +\
    ['не ' + sub2 for sub2 in ['бол','выш','длинн','превыш','превосход']]
    __recognized_min = ['мин', 'бол', 'выш', 'длинн']
    __recognized_min_inverted = ['не' + sub1 + sub2 for sub1 in [' ', ' должен быть ', ' должен '] for sub2 in ['меньш','ниже','короч','мене']] + \
    ['не ' + sub2 for sub2 in ['меньш','ниже','короч','менее']]
    __recognized_volume = ['объем', 'объём', 'размер', 'содержа', 'состоя', 'количеств']
    __recognized_volume_unit = ['cтр', 'знаков без пробелов', 'символов без пробелов', 'знаков с пробелам', 'символов с пробелам', 'знак', 'символ', 'слов']
                        ...
                        for i, word in enumerate(word_list):
                            ...
                            if i < len(word_list)-1:
                                if word.isdigit():
                                    ...
                                    unit_found, unit = self.__text_contains_word_from_list_with_word(word_list[i+1], self.__recognized_volume_unit)
...
#  !!
    def __get_style_data(self, text):
        ...
        for i, sent in enumerate(list_of_sentences):
            ...
            direction_exists, direction = sent.__text_contains_word_from_list_with_word(sent, self.__recognized_direction)
                    ...
                    if direction == 'верх':
                        for j in range(max(c, i-3), min(i+5, len(word_list))):
                            if word_list[j][0].isdigit() and data_fields['top'] == None:
                                ...
                                unit_exists, unit = self.__text_contains_word_from_list(word_list[j], self.__recognized_linear_unit)
                                if(unit_exists):
                                    data_fields['unit'] = unit
                                elif(j < len(word_list)-1):
                                    unit_exists, unit = self.__text_contains_word_from_list(word_list[j+1], self.__recognized_linear_unit)
                                    if(unit_exists):
                                        data_fields['unit'] = unit
                                c = j
                                break
